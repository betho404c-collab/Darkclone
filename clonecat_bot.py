...
def _telethon_sync(method, owner=None, *args, **kwargs):
    """Executa um método Telethon e aguarda a coroutine quando necessário."""
...
def clone_selected(client, origin_chat, destination_chat, topic_map, selected_topic_title,
    ...
    api_id = int(os.environ.get("TELEGRAM_API_ID", os.environ.get("API_ID", "0")))
    api_hash = os.environ.get("TELEGRAM_API_HASH", os.environ.get("API_HASH", ""))
    ...
    # Só depois de obter TODO o histórico consultamos o resume.
    # Isso mantém a ordem do fluxo desejado: histórico -> total -> resume -> clonagem.
    if resume_data is None:
        resume_data = load_resume(int(origin_chat))
    resuming = resume_data is not None
    if resuming:
        notify(f"♻️ Resume encontrado após o histórico: {len(resume_data.get('cloned_msg_ids', []))} de {resume_data.get('total_msgs', total_messages)} mensagens já clonadas.")
                        ...
                        if media_obj is not None:
                            ...
                            if next_media_idx is not None:
                                ...
                                def _bg_download(mo=nm_mo, fp=nm_fp, mid=nm.id, mt=nm_mt,
                                    ...
                                    _telethon_sync(bg.connect, bg)
                                    try:
                                        _download_sync(bg, mo, fp, mid, mt)
                                    finally:
                                        _telethon_sync(bg.disconnect, bg)
                                        loop.close()
...
ADMIN_USER_ID = int(os.getenv("ADMIN_USER_ID", os.getenv("ADMIN_IDS", "0").split(",")[0] or "0"))
...
def create_client():
    session = Path(SESSION_FILE).read_text().strip() if os.path.exists(SESSION_FILE) else ""
    return TelegramClient(
        StringSession(session), API_ID, API_HASH,
        connection_retries=20, request_retries=20, retry_delay=5,
        auto_reconnect=True,
    )
...
async def begin_connect(query, context):
    context.user_data.clear()
    context.user_data["state"] = "phone"
...
def send_code_sync(phone):
    ...
    _telethon_sync(client.connect, client)
    if client.is_user_authorized():
        save_session(client)
        _telethon_sync(client.disconnect, client)
...
def finish_login_sync(phone, code, phone_code_hash, password=None):
    client = create_client()
    _telethon_sync(client.connect, client)
    try:
        try:
...
async def handle_phone(update, context):
    ...
    try:
        result = await asyncio.to_thread(send_code_sync, phone)
        if result.get("authorized"):
            context.user_data.clear()
            await update.message.reply_text("🟢 Conta conectada e sessão salva no Volume do Railway.", reply_markup=menu_keyboard())
            return
        context.user_data.update(state="code", phone=phone, phone_code_hash=result["phone_code_hash"])
        await update.message.reply_text("🔐 Envie o código que o Telegram acabou de enviar.")
    except PhoneNumberInvalidError:
        await update.message.reply_text("❌ Número inválido. Envie novamente.")
    except FloodWaitError as e:
        await update.message.reply_text(f"⏳ Aguarde {e.seconds}s antes de tentar novamente.")
    except Exception as e:
        await update.message.reply_text(f"❌ Erro: <code>{html.escape(str(e))}</code>", parse_mode=ParseMode.HTML)
...
async def handle_code(update, context):
    ...
    if not phone or not phone_code_hash:
        context.user_data.clear()
        await update.message.reply_text("❌ Login expirado. Use /connect novamente.")
        return
    try:
        result = await asyncio.to_thread(finish_login_sync, phone, code, phone_code_hash)
        if result.get("password_required"):
            context.user_data.update(state="password", code=code)
            await update.message.reply_text("🔐 Sua conta usa 2FA. Envie a senha 2FA.")
            return
        context.user_data.clear()
        await update.message.reply_text("🟢 Conta conectada e sessão salva no Volume do Railway.", reply_markup=menu_keyboard())
    except PhoneCodeExpiredError:
        context.user_data.clear()
        await update.message.reply_text("⌛ O código expirou. Use /connect para gerar outro.")
    except PhoneCodeInvalidError:
        await update.message.reply_text("❌ Código incorreto. Envie novamente ou /cancel.")
    except FloodWaitError as e:
        await update.message.reply_text(f"⏳ Aguarde {e.seconds}s.")
    except Exception as e:
        await update.message.reply_text(f"❌ Erro no login: <code>{html.escape(str(e))}</code>", parse_mode=ParseMode.HTML)
...
async def handle_password(update, context):
    ...
    if not all([phone, code, phone_code_hash]):
        context.user_data.clear()
        await update.message.reply_text("❌ Login incompleto. Use /connect novamente.")
        return
    try:
        result = await asyncio.to_thread(finish_login_sync, phone, code, phone_code_hash, password)
        if result.get("authorized"):
            context.user_data.clear()
            await update.message.reply_text("🟢 Conta conectada e sessão salva no Volume do Railway.", reply_markup=menu_keyboard())
    except Exception as e:
        await update.message.reply_text(f"❌ Erro na senha 2FA: <code>{html.escape(str(e))}</code>", parse_mode=ParseMode.HTML)
...
def get_entity_info_sync(chat_id):
    client = create_client()
    _telethon_sync(client.connect, client)
    try:
...
def list_topics_sync(chat_id):
    client = create_client()
    _telethon_sync(client.connect, client)
    try:
        entity = client.get_entity(int(chat_id))
        if not isinstance(entity, Channel) or not getattr(entity, "forum", False):
            raise ValueError("O destino precisa ser um supergrupo com Fórum habilitado.")
        result = client(GetForumTopicsRequest(peer=entity, offset_date=None, offset_id=0, offset_topic=0, limit=100))
        topics = []
        for t in getattr(result, "topics", []) or []:
            tid = getattr(t, "id", 0)
            title = getattr(t, "title", None) or ("Geral" if tid == 1 else f"Tópico {tid}")
        ...
        return {"title": getattr(entity, "title", str(entity.id)), "topics": topics}
    finally:
        _telethon_sync(client.disconnect, client)
...
async def handle_origin(update, context):
    try:
        origin = int(update.message.text.strip())
        info = await asyncio.to_thread(get_entity_info_sync, origin)
    except ValueError:
        await update.message.reply_text("❌ O ID deve ser numérico.")
        return
    except Exception as e:
        await update.message.reply_text(f"❌ Não consegui acessar a origem.\n<code>{html.escape(str(e))}</code>", parse_mode=ParseMode.HTML)
        return
    context.user_data.update(origin=origin, state="destination")
    await update.message.reply_text(
        f"✅ <b>Origem encontrada.</b>\n\n{html.escape(info['title'])}\nTipo: {html.escape(info['type_label'])}\nID: <code>{origin}</code>\n\nAgora envie o ID numérico do grupo de destino.",
...
async def handle_destination(update, context):
    try:
        dest = int(update.message.text.strip())
        data = await asyncio.to_thread(list_topics_sync, dest)
    except ValueError:
        await update.message.reply_text("❌ O ID deve ser numérico.")
        return
    except Exception as e:
        ...
        return
    context.user_data.update(destination=dest, topics=data["topics"], state="topic")
    rows = []
    for i, topic in enumerate(data["topics"]):
        label = f"{i+1}. {topic['title']}" + (" 🔒" if topic["closed"] else "")
...
async def topic_callback(query, context, idx):
    ...
    if idx < 0 or idx >= len(topics):
        await query.answer("Tópico inválido.", show_alert=True)
        return
    topic = topics[idx]
    context.user_data.update(topic_map={0: topic["anchor"]}, topic_title=topic["title"], state="content")
    ...
    await query.edit_message_text(f"🎯 Tópico selecionado: <b>{html.escape(topic['title'])}</b>\n\nEscolha o conteúdo:", parse_mode=ParseMode.HTML, reply_markup=kb)
...
async def content_callback(query, context, value):
    ...
    # IMPORTANTE: não consulta resume aqui. O motor consulta o histórico primeiro.
    context.user_data["resume_data"] = None
    origin = context.user_data["origin"]
    dest = context.user_data["destination"]
    title = context.user_data["topic_title"]
...
class StatusBridge:
    ...
    def __call__(self, text):
        ...
        if not text or text == self.last_text:
            return
        now = time.time()
        if now - self.last < 1.0 and "Total de mensagens" not in text:
            return
        self.last = now
        self.last_text = text
        if len(text) > 3500:
            text = text[-3500:]
        future = asyncio.run_coroutine_threadsafe(self.edit(text), self.loop)
...
def _run_engine_sync(origin, dest, topic_map, title, content, resume_data=None, status_callback=None):
    """Ponte síncrona entre o bot assíncrono e o motor clone_selected()."""
    client = None
    stage = "criando cliente Telethon"
    try:
        client = create_client()
        stage = "conectando ao Telegram"
        _telethon_sync(client.connect, client)

        stage = "verificando autorização da sessão"
        if not client.is_user_authorized():
            raise RuntimeError("A sessão Telethon não está autorizada. Use /connect para conectar a conta.")

        stage = "validando chat de origem"
        client.get_entity(int(origin))
        stage = "validando fórum de destino"
        client.get_entity(int(dest))

        stage = "obtendo histórico de mensagens"
        result = clone_selected(
            client,
            int(origin),
            int(dest),
            topic_map,
        ...
        logging.error("Falha na etapa '%s': %s\\n%s", stage, exc, tb)
        raise RuntimeError(f"Falha na etapa: {stage}\\n{type(exc).__name__}: {exc}\\n\\n{tb}") from exc
    finally:
        if client is not None:
            try:
                _telethon_sync(client.disconnect, client)
            except Exception:
                logging.exception("Erro ao desconectar cliente Telethon")


async def start_clone(query, context):
    if JOB_LOCK.locked():
        await query.answer("Já existe uma clonagem em execução.", show_alert=True)
        return
    origin = context.user_data.get("origin")
    dest = context.user_data.get("destination")
    topic_map = context.user_data.get("topic_map")
    title = context.user_data.get("topic_title")
    content = context.user_data.get("content_types")
    ...
    async with JOB_LOCK:
        try:
            # resume_data=None de propósito: clone_selected obterá o histórico primeiro
            # e só então carregará o resume existente.
            result = await asyncio.to_thread(_run_engine_sync, origin, dest, topic_map, title, content, None, status)
            await query.message.reply_text(
                f"✅ <b>Clonagem concluída.</b>\n\n📊 Total: {result.get('total_messages', 0)}\n✅ Clonadas: {result.get('cloned_count', 0)}\n⏭️ Puladas: {result.get('skipped_count', 0)}\n❌ Falhas: {result.get('failed_count', 0)}",
                parse_mode=ParseMode.HTML,
                reply_markup=menu_keyboard(),
            )
        except Exception as e:
            logging.exception("Erro na clonagem")
            detail = str(e)
            if len(detail) > 3200:
                detail = detail[:3200] + "\n..."
            await query.message.reply_text(
                f"❌ <b>Erro real do motor:</b>\n<code>{html.escape(detail)}</code>\n\nO resume permanece salvo.",
                parse_mode=ParseMode.HTML,
                reply_markup=menu_keyboard(),
            )
...
async def callback(update, context):
    ...
    data = query.data
    if data == "new":
        await new_clone(query, context)
    elif data == "connect":
        await begin_connect(query, context)
    elif data == "confirm":
        await start_clone(query, context)
    elif data == "cancel":
        context.user_data.clear()
        await query.message.reply_text("❌ Cancelado.", reply_markup=menu_keyboard())
    elif data.startswith("topic:"):
        await topic_callback(query, context, int(data.split(":", 1)[1]))
    elif data.startswith("content:"):
        await content_callback(query, context, data.split(":", 1)[1])
    elif data == "status":
        origin = context.user_data.get("origin")
        resume = await asyncio.to_thread(load_resume, origin) if origin else None
        if resume:
...
async def text_router(update, context):
    ...
    if state == "phone":
        ...
        await handle_origin(update, context)
    elif state == "destination":
        await handle_destination(update, context)
    else:
...
def main_bot():
    ...
    print("CLONECAT BOT V4 - HISTORICO ANTES DO RESUME")
