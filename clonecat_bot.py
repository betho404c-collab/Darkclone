...
async def topic_callback(query, context, idx):
    ...
    topic = topics[idx]
    if topic.get("closed"):
        await query.answer("Esse tópico está fechado.", show_alert=True)
        return
...
def _format_exception_for_user(exc):
    text = f"{type(exc).__name__}: {exc}"
    if len(text) > 3500:
        text = text[:3500] + "\n...[cortado]"
    return text

def _run_engine_sync(origin, dest, topic_map, title, content_types, resume_data=None, status_callback=None):
    """
    Ponto único de entrada do bot para o motor original.

    A V4 chamava esta função em start_clone(), mas ela não existia no arquivo.
    Isso fazia a execução morrer antes de clone_selected() e, portanto, antes
    de client.iter_messages().
    """
    client = create_client()
    try:
        client.connect()
        if not client.is_user_authorized():
            raise RuntimeError(
                "A conta Telethon não está autorizada. Use /connect para autenticar a conta."
            )

        if status_callback:
            status_callback("🔌 Conta Telethon conectada. Iniciando motor...")

        # Mantém a ordem intencional da V4: o motor obtém o histórico primeiro
        # e somente depois consulta o resume.
        result = clone_selected(
            client=client,
            origin_chat=int(origin),
            destination_chat=int(dest),
            topic_map=topic_map,
        ...
        logging.exception("Erro no _run_engine_sync")
        if status_callback:
            status_callback(f"❌ Falha no motor: {type(exc).__name__}: {exc}")
        raise
    finally:
...
async def start_clone(query, context):
    ...
    if not all([origin, dest, topic_map is not None, content]):
        ...
        return
    await query.edit_message_text("🚀 <b>Clonagem iniciada!</b>\n\n🔌 Conectando à conta Telethon...", parse_mode=ParseMode.HTML)
    ...
    async with JOB_LOCK:
        try:
            ...
            await query.message.reply_text(
                f"⏳ <b>Telegram pediu espera.</b>\n\nAguarde {e.seconds}s e tente novamente.\n\n<code>{html.escape(repr(e))}</code>",
                parse_mode=ParseMode.HTML,
                reply_markup=menu_keyboard(),
            )
        except Exception as e:
