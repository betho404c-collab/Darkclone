...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if state["step"] == "destination":
            ...
            # IMPORTANTE: preserve o ID que o usuário enviou (ex.: -100123...).
            # entity.id de Channel é o ID interno positivo; o clonecat original
            # precisa do peer ID -100... quando inicia em um processo separado.
...
async def launch_clone(message, resume=False):
    ...
    try:
        logging.info("Iniciando clonecat: script=%s cwd=%s inputs=%r", SCRIPT, BASE_DIR, inputs)
        logging.info("Arquivos: config=%s session=%s resume=%s", CONFIG_FILE, SESSION_FILE, RESUME_FILE)
...
async def monitor_job(proc, message):
    ...
    if rc != 0:
        tail_now = "\n".join(output_lines[-20:]) if output_lines else "(sem saída capturada)"
        logging.error("clonecat terminou com código %s. Últimas linhas:\n%s", rc, tail_now)
