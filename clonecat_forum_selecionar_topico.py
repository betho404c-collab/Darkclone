...
# No Railway, /data é o volume persistente; mídias temporárias ficam fora dele.
BASE_DATA_DIR = os.getenv("DATA_DIR", ".")
TEMP_DIR = os.getenv("TEMP_MEDIA_DIR", "/tmp/clonecat_temp_media")
os.makedirs(TEMP_DIR, exist_ok=True)

# Arquivo de retomada (resume)
RESUME_FILE = os.path.join(BASE_DATA_DIR, "resume_forum.json")
...
def main():
    ...
    # Carrega/gerencia sessao como string para compartilhar entre clientes
    session_file = os.path.join(BASE_DATA_DIR, "session.txt")
    ...
    resuming = False
    if resume_data:
        print(f"\n{Fore.CYAN}Retomada encontrada para este chat:{Style.RESET_ALL}")
        print(f"  Destino: {resume_data['destination_chat']}")
        print(f"  Já clonadas: {len(resume_data['cloned_msg_ids'])} de {resume_data['total_msgs']} msgs")
        resp = input("\nRetomar de onde parou? (s/n): ").strip().lower()
        if resp == "s":
            resuming = True
            destination_chat = resume_data["destination_chat"]
            # Garante que a entidade de destino esta no cache da sessao
            try:
                client.get_entity(destination_chat)
            except Exception:
                # Tenta com prefixo -100 (supergrupo/canal)
                try:
                    client.get_entity(int(f"-100{destination_chat}"))
                    destination_chat = int(f"-100{destination_chat}")
                except Exception:
                    print(f"  {Fore.YELLOW}Aviso: nao foi possivel acessar o destino {destination_chat}{Style.RESET_ALL}")
    ...
    if not resuming:
        destination_chat, topic_map, selected_topic_title = select_destination_topic(client)
    else:
        # Resume antigo pode ter sido criado pelo script anterior, que
        # criava um grupo/tópico automaticamente. Nesse caso, pedimos
        # novamente o destino e o tópico para evitar enviar para o lugar errado.
        saved_destination = validate_saved_destination(client, destination_chat)
        if saved_destination is None or 0 not in topic_map:
            print(f"\n{Fore.YELLOW}O destino salvo no resume não é compatível com esta versão.{Style.RESET_ALL}")
            print("Vamos selecionar novamente o Fórum e o tópico de destino.")
            destination_chat, topic_map, selected_topic_title = select_destination_topic(client)
        else:
            destination_chat = saved_destination
            topic_anchor = topic_map.get(0)
            print(f"\n{Fore.GREEN}Destino carregado do resume: {destination_chat}{Style.RESET_ALL}")
            print(f"  Âncora do tópico: {topic_anchor}")
            selected_topic_title = resume_data.get("destination_topic_title", "")

    # ================================================================
    # PASSO 3: Seleciona tipo de conteúdo
    # ================================================================
    if not resuming:
